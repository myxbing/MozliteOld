...
namespace Mozlite.Extensions.Identity
{
        ...
        where TUserToken : IdentityUserToken, new()
    {
        ...
        {
            return _users.FindAsync(u => u.UserId == userId.AsInt32(), cancellationToken);
        }

        public virtual Task<TUser> FindByLoginAsync(string loginProvider, string providerKey, CancellationToken cancellationToken)
        {
            return _users.Query.InnerJoin<TUserLogin>((u, l) => u.UserId == l.UserId)
        ...
        {
            var userClaims = await _userClaims.LoadAsync(uc => uc.UserId == user.UserId, cancellationToken);
        ...
        {
            var userLogins = await _userLogins.LoadAsync(ul => ul.UserId == user.UserId, cancellationToken);
            return
        ...
        {
            var users = await _users.Query.InnerJoin<TUserClaim>((u, c) => u.UserId == c.UserId)
        ...
        {
            roleName = roleName.ToUpper();
            var users = await _users.Query.InnerJoin<TUserRole>((u, x) => u.UserId == x.UserId)
        ...
        public async virtual Task<bool> IsInRoleAsync(TUser user, string roleName, CancellationToken cancellationToken)
        {
            var userRole = await _userRoles.Query.InnerJoin<TRole>((ur, r) => ur.RoleId == r.RoleId)
        ...
        {
            return _roles.Query.InnerJoin<TUserRole>((r, ur) => r.RoleId == ur.RoleId)
