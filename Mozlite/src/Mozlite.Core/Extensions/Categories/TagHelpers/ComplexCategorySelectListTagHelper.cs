...
namespace Mozlite.Extensions.Categories.TagHelpers
{
    /// <summary>
    /// 多级分类下拉列表。
    /// </summary>
        ...
        where TCategory : ComplexCategoryBase<TCategory>, new()
    {
        ...
        protected override void Init(List<SelectListItem> items, TagHelperContext context)
        {
            ...
            if (categories != null)
            {
                ...
                {
                    AppendCategory(items, categories[i], id, i + 1 == categories.Count);
                }
            }
        }
        ...
        private string SelectText(TCategory category, bool isEnd, string header)
        {
        ...
        private void AppendCategory(List<SelectListItem> items, TCategory category, int? value, bool isEnd, string header = null)
        {
            ...
            items.Add(new SelectListItem { Text = SelectText(category, isEnd, header), Value = category.Id.ToString(), Selected = value == category.Id });
